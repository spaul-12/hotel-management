...
func ValidateRegister(u *models.User) *models.UserErrors {
	...
	if !(len(u.Password) >= 6 && valid.HasLowerCase(u.Password) && valid.HasUpperCase(u.Password) && re.MatchString(u.Password)) {
		e.Err, e.Password = true, "Length of password should be atleast 6 and it must be a combination of uppercase letters, lowercase letters and numbers"
	}
